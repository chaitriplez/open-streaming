 ...
 *
 * <p>By contract on start of each cycle {@link AtomicRateLimiter} should set {@link
 * State#activePermissions} to {@link RateLimiterConfig#limitForPeriod}. For the {@link
...
public class AtomicRateLimiter implements RateLimiter {
  ...
  // Tuning: change start time to begin of the day
  private static final long nanoTimeStart = nanoTime() - LocalTime.now().toNanoOfDay();
  ...
  public AtomicRateLimiter(
    ...
    state =
        new AtomicReference<>(
            new State(rateLimiterConfig, 0, rateLimiterConfig.getLimitForPeriod(), 0));
  ...
  public void changeTimeoutDuration(final Duration timeoutDuration) {
    RateLimiterConfig newConfig =
        RateLimiterConfig.from(state.get().config).timeoutDuration(timeoutDuration).build();
    state.updateAndGet(
        currentState ->
            new State(
                newConfig,
                currentState.activeCycle,
                currentState.activePermissions,
  ...
  public void changeLimitForPeriod(final int limitForPeriod) {
    RateLimiterConfig newConfig =
        RateLimiterConfig.from(state.get().config).limitForPeriod(limitForPeriod).build();
    state.updateAndGet(
        currentState ->
            new State(
                newConfig,
   ...
   * while before try one more time. This technique was originally described in this <a
   * href="https://arxiv.org/abs/1305.5800">paper</a> and showed great results with {@link
   ...
   * expected value. It differs from {@link AtomicReference#updateAndGet(UnaryOperator)} by constant
   * back off. It means that after one try to {@link AtomicReference#compareAndSet(Object, Object)}
   * this method will wait for a while before try one more time. This technique was originally
   * described in this <a href="https://arxiv.org/abs/1305.5800">paper</a> and showed great results
   * with {@link AtomicRateLimiter} in benchmark tests.
   *
   ...
   * time duration that you should wait for the given number of permits and reserves it for you, if
   * you'll be able to wait long enough.
   *
  ...
  private State calculateNextState(
      final int permits, final long timeoutInNanos, final State activeState) {
   ...
   * @param currentCycle current {@link AtomicRateLimiter} cycle @return nanoseconds to wait for the
   *     next permission
   */
   ...
   */
  private State reservePermissions(
  ...
  /**
   * If nanosToWait is bigger than 0 it tries to park {@link Thread} for nanosToWait but not longer
   * then timeoutInNanos.
   *
   ...
   *
   * <p>If the current thread is {@linkplain Thread#interrupted} while waiting for a permit then it
   * won't throw {@linkplain InterruptedException}, but its interrupt status will be set.
   *
   * @param nanosToWait nanoseconds caller need to wait
   ...
   *
   * <ul>
   *   <li>activeCycle - {@link AtomicRateLimiter} cycle number that was used by the last {@link
   *       AtomicRateLimiter#acquirePermission()} call.
   *       <p>
   *   <li>activePermissions - count of available permissions after the last {@link
   *       AtomicRateLimiter#acquirePermission()} call. Can be negative if some permissions where
   *       reserved.
   *       <p>
   *   <li>nanosToWait - count of nanoseconds to wait for permission for the last {@link
   *       AtomicRateLimiter#acquirePermission()} call.
   * </ul>
   */
